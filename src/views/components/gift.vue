<template>
  <div :class="['gift-container', displayType, { 'autoTurn': isRunning }]">
    <div
      ...
      v-for="(gift, index) in remindGifts"
      :key="index"
  ...
  methods: {
    removeGift () {
      this.config.gifts.forEach((gift, index) => {
        if (this.history.includes(gift.name) === false) {
          this.remindGifts.push(gift)
        }
      })
    },
    logGiftsDeg () {
      // 紀錄獎品角度
      this.remindGifts.forEach((gift, index) => {
        this.giftsDeg[index] = {
    ...
    setConfig () {
      ...
      // kodak自定
      this.$el.style.setProperty('--left', `${this.config.left}px`)
      this.$el.style.setProperty('--top', `${this.config.top}px`)
      this.$el.style.setProperty('--position', `${this.config.position}`)
    },
    autoTurn () {
      // 取得隨機角度(預設至少跑5圈)
